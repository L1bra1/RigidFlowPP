"""
To train models on occluded FlyingThings3D.
...
def main():
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.5, last_epoch=init_epoch - 1)
    ...
    for epoch in range(init_epoch, args.epochs):
        ...
        for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader), smoothing=0.9):
            pos1, pos2, norm1, norm2, flow, mask1, _, voxel_list = data
            # move to cuda
            pos1 = pos1.cuda()
            pos2 = pos2.cuda()
            norm1 = norm1.cuda()
        ...
        eval_epe3d, eval_acc3d, eval_epe3d_all = eval_sceneflow(model.eval(), val_loader)
        str_out = 'EPOCH %d %s mean epe3d_all: %f mean epe3d: %f  mean acc3d: %f' % (epoch, blue('eval'),
                                                                                     eval_epe3d_all, eval_epe3d,
                                                                                     eval_acc3d)
        print(str_out)
        logger.info(str_out)
        ...
        torch.save(model.module.state_dict(),
                   '%s/%s_%.3d_%.4f.pth' % (checkpoints_dir, args.model_name, epoch, eval_epe3d))
...
def eval_sceneflow(model, loader):
    ...
    for batch_id, data in tqdm(enumerate(loader), total=len(loader), smoothing=0.9):
        pos1, pos2, norm1, norm2, flow, mask1, _, _ = data
        # move to cuda
        pos1 = pos1.cuda()
        pos2 = pos2.cuda()
        norm1 = norm1.cuda()
        ...
        with torch.no_grad():
            ...
            epe_3d_all, _, _, _ = evaluate_3d(pred_flows[0].permute(0, 2, 1).detach().cpu().numpy(),
        ...
        metrics['epe_3d_all'].append(epe_3d_all)

    mean_epe3d_all = np.mean(metrics['epe_3d_all'])
    mean_epe3d = np.mean(metrics['epe_3d'])
    mean_acc3d = np.mean(metrics['acc_3d'])

    return mean_epe3d, mean_acc3d, mean_epe3d_all
